...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query(value = "SELECT * FROM task t WHERE t.user_id=?1 and t.completed_at IS NOT NULL ORDER BY completed_at DESC,(CASE \n" +
            "    WHEN t.importance='vital' THEN 1 \n" +
            "    WHEN t.importance='important' THEN 2\n" +
            "    WHEN t.importance='standard' THEN 3\n" +
            "    ELSE 4\n" +
            "END), deadline ASC;", nativeQuery = true)
    Iterable<Task> findTasksByUserAndCompletedAtIsNotNullAtOrderByCompletedAtDesc(Long userId);
